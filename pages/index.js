...
const todoForm = new TodoForm({
  ...
  handleFormSubmit: (inputData, editTodoInfo) => {
    ...
    if (editingTodo) {
      api.editTodo(inputData.todo, editTodoInfo.id)
        .then(res => {
          editTodoInfo.element.querySelector('.todo__text').textContent = res.name;
          editingTodo = null;
        })
        .catch(err => console.log(err))
        .finally(() => {
